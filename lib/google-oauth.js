...
function acquireOAuthFlowLock() {
  ...
  function attempt(n) {
    if (n >= 100) return Promise.resolve(owner);
    var now = Date.now();
  ...
  return new Promise(function (resolve) {
    chrome.identity.launchWebAuthFlow(
      ...
      function (redirectUrl) {
        ...
        if (le) console.warn('[auth] launchWebAuthFlow callback error:', le.message);
        resolve(parseGoogleAuthRedirect(le && le.message, redirectUrl));
      }
    );
  });
}
